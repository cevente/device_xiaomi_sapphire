...
class XiaomiSm6225UdfpsHandler : public UdfpsHandler {
  ...
  private:
    ...
    void setHbmOn() {
        {
            ...
            if (disp_fd_.get() >= 0) {
                ...
                if (ioctl(disp_fd_.get(), MI_DISP_IOCTL_SET_LOCAL_HBM, &req) < 0) {
                    LOG(ERROR) << "Failed to set local HBM ON: " << strerror(errno);
                } else {
                    LOG(INFO) << "💡 Local HBM enabled via ioctl";
                }
            }
        }
    ...
    void setHbmOff() {
        {
            ...
            if (disp_fd_.get() >= 0) {
                ...
                if (ioctl(disp_fd_.get(), MI_DISP_IOCTL_SET_LOCAL_HBM, &req) < 0) {
                    LOG(ERROR) << "Failed to set local HBM OFF: " << strerror(errno);
                } else {
                    LOG(INFO) << "💡 Local HBM disabled via ioctl";
                }
            }
        }
    ...
    void fodPressMonitorThread() {
        ...
        while (isRunning.load()) {
            ...
            if (currentFodStatus != FOD_STATUS_ON) {
                LOG(DEBUG) << "UDFPS: Ignoring hardware FOD event because FOD status is OFF";
                readBool(fd);
                continue;
            }
    ...
    int getFodStatus() {
        ...
        int buf[MAX_BUF_SIZE] = {MI_DISP_PRIMARY, Touch_Fod_Enable, 0};
        if (ioctl(touch_fd_.get(), TOUCH_IOC_GET_CUR_VALUE, &buf) < 0) {
            LOG(ERROR) << "Failed to get FOD status: " << strerror(errno);
            return -1;
        }
        return buf[2];
    }
